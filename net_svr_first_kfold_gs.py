...
outputs_first = np.array([i[:1000] for i in outputs])
# outputs_pref = np.array([i[1000:] for i in outputs])

print outputs_first.shape
# print outputs_pref.shape

# outputs2 = np.array(outputs)


X_train, X_test, y_train, y_test = train_test_split(outputs_first, views2, test_size=0.2, random_state=42)
...
pickle.dump(clf, open('svr_test_first_after_gs_25.07.sk', 'wb'))
...
### KFold for first:
kf = KFold(outputs_first.shape[0], n_folds=5)

for train, test in kf:
    print "Next iterration..."
    sys.stdout.flush()

    X_train, X_test, y_train, y_test = outputs_first[train], outputs_first[test], views2[train], views2[test]
